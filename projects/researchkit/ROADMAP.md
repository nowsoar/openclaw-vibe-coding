...
- [x] 小红书数据源（Playwright + Cookie 模式）
- [x] 正文补充抓取（对有价值的文章获取全文）
- [x] 更多报告模板（用户研究、技术评测、政策分析）
- [x] 模板在线编辑（支持用户自定义 AI Prompt）
- [x] 引用验证（确认来源链接可访问）
- [x] 内容质量评分（自动判断文章信息密度）
- [x] 飞书文档输出
- [x] PDF 输出
...
- [x] 数据库迁移：SQLite（保持简单，多用户 SQLite 足够）
- [x] 用户表设计（User / Organization）
- [x] JWT 认证（注册 / 登录 / 刷新 Token）
- [x] 邮箱注册 + 密码重置
- [x] Google OAuth 登录
- [x] 飞书 OAuth 登录
- [x] 数据隔离（每个用户的任务/数据源/报告独立）
- [x] 权限控制（个人版 / 团队版）
...
- [x] 增量更新（追加新内容到已有报告）
- [x] 事件触发（关键词热度突增时触发调研）
- [x] 团队共享（调研任务 / 模板 / 数据源共享）
- [x] 通知推送（飞书 / 邮件 / Webhook）
- [x] 插件机制（社区贡献新数据源 / 处理器）
- [x] 数据源插件市场（知乎、B站、Twitter 等）
- [x] 报告模板市场（各行业专业模板）
